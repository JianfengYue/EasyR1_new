...
from ...utils.sft_utils import compute_sft_loss_tf
...
class DataParallelPPOActor(BasePPOActor):
    def __init__(
        ...
        self.processed_sft_uids: set[str] = set()
    ...
    def update_policy(self, data: DataProto) -> Dict[str, Any]:
        ...
        self.processed_sft_uids.clear()

        temperature = data.meta_info["temperature"]  # temperature must be in the data.meta_info to avoid slient error
        select_keys = ["input_ids", "attention_mask", "position_ids", "responses", "response_mask", "sft_input_ids", "sft_attention_mask", "labels"]
        select_keys.extend(["old_log_probs", "ref_log_probs", "advantages"])
        non_tensor_select_keys = ["multi_modal_inputs", "ground_truth_input_ids", "uid"]
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for micro_batch in micro_batches:
                    ...
                    if self.config.sft_enable:
                        sft_loss = None
                        uids = micro_batch.non_tensor_batch.get("uid")

                        if uids is not None:
                            uid_list = [str(u) for u in uids.tolist()]
                            keep_indices = [i for i, uid in enumerate(uid_list) if uid not in self.processed_sft_uids]
                            if keep_indices:
                                unique_batch = micro_batch[keep_indices]
                                sft_loss = compute_sft_loss_tf(self.actor_module, unique_batch)
                                for idx in keep_indices:
                                    self.processed_sft_uids.add(uid_list[idx])
                        else:
                            # 没有 uid，则直接对当前 micro_batch 计算一次（无去重）
                            sft_loss = compute_sft_loss_tf(self.actor_module, micro_batch)

                        # 只要本步算出了 sft_loss，就混合并记录；否则用上一次的数值做日志填充，保证曲线连续
                        if sft_loss is not None:
                            sft_loss_val = float(sft_loss.detach())
                            batch_metrics["actor/sft_loss"] = sft_loss_val
                            self.last_sft_loss = sft_loss_val
                            loss = 0.5 * loss + 0.5 * sft_loss
                        elif self.last_sft_loss is not None:
                            batch_metrics["actor/sft_loss"] = self.last_sft_loss
