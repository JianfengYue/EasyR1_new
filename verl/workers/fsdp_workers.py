...
class FSDPWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_sft_loss(self, data: DataProto) -> DataProto:
        assert self._has_actor, "Actor is not initialized in this worker."

        # ---------- 1) 先保存原始 uid，并构建稳定的去重/还原映射 ----------
        orig_uids = None
        keep_indices = None
        map_idx = None
        if "uid" in data.non_tensor_batch:
            uids = np.asarray(data.non_tensor_batch["uid"], dtype=object)
            orig_uids = uids  # 原始长度 (包含5次 rollout 的重复)

            # 构建：keep_indices（保留首个出现的索引），map_idx（原始每个位置 -> 去重后的位置）
            seen = {}
            keep_indices_list = []
            map_idx_list = np.empty(len(uids), dtype=np.int64)
            for i, u in enumerate(uids):
                if u in seen:
                    map_idx_list[i] = seen[u]  # 指向已存在的 unique 位置
                else:
                    pos = len(keep_indices_list)  # unique 中的新位置
                    seen[u] = pos
                    keep_indices_list.append(i)
                    map_idx_list[i] = pos

            keep_indices = np.asarray(keep_indices_list, dtype=np.int64)
            map_idx = torch.as_tensor(map_idx_list, dtype=torch.long, device=torch.cuda.current_device())

            # 若确有重复，则对 data 进行去重切片（和你原来的逻辑一致）
            if len(keep_indices) != len(uids):
                data.batch = data.batch[keep_indices]
                for k, v in data.non_tensor_batch.items():
                    data.non_tensor_batch[k] = v[keep_indices]
        ...
        # ---------- 3) （可选）FSDP 上卡 ----------
        if getattr(self, "_use_param_offload", False):
        ...
        with self.ulysses_sharding_manager:
            ...
            with torch.inference_mode():
                sft_loss_unique = self.actor.compute_sft_loss(data=data)  # [B_unique]
            sft_loss_unique = self.ulysses_sharding_manager.postprocess_data(
                DataProto.from_dict(tensors={"sft_loss": sft_loss_unique})
            ).batch["sft_loss"]  # 取回张量

        # ---------- 5) 复原到原始长度（若有重复） ----------
        if map_idx is not None and orig_uids is not None and keep_indices is not None:
            # 按 map_idx 从 unique loss 选取，得到与 orig_uids 等长的张量
            sft_loss_full = sft_loss_unique[map_idx]   # shape = [len(orig_uids)]
            out_tensors = {"sft_loss": sft_loss_full}
            out_non_tensors = {"uid": orig_uids}
        else:
            out_tensors = {"sft_loss": sft_loss_unique}
            out_non_tensors = {"uid": data.non_tensor_batch.get("uid")} if "uid" in data.non_tensor_batch else None

        # ---------- 6) FSDP reshard & 下卡 ----------
        if self.world_size > 1 and hasattr(self, "fsdp_module"):
            self.fsdp_module._handle.reshard(True)
        if getattr(self, "_use_param_offload", False):
            offload_fsdp_model(self.fsdp_module)

        # ---------- 7) 回 CPU ----------
        output = DataProto.from_dict(tensors=out_tensors, non_tensors=out_non_tensors).to("cpu")
        return output